...
class AddUsedCarModel
{
    ...
    public function showEditCar(){
        if(isset($_GET['id']) && !empty($_GET['id'])){
            ...
            $sql = "SELECT * FROM used_cars WHERE id=:id";
            $value = ['id' => $id];
    ...
    public function editCar(){
        if(isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price']) && isset($_POST['year']) && !empty($_POST['year']) && isset($_POST['select']) && !empty($_POST['select']) && isset($_FILES['images']['name'][0]) && !empty($_FILES['images']['name'][0]) && isset($_POST['specifications_en']) && !empty($_POST['specifications_en']) && isset($_POST['specifications_ar']) && !empty($_POST['specifications_ar']) && isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])){
            ...
            if($result = DataBase::$db->prepare($sql,$value)){
                ...
                $dir = 'assets/images/';
                $imgName = Validation::files("images",$allowEx,$dir,"used_car");
                ...
                elseif($imgName == 3){
                    ...
                    $activeimgName = Validation::file("image",$allowEx,$dir,"used_car");
                ...
                elseif($activeimgName == 3){
                    ...
                    $km = Validation::validateInput($_POST['km']);

                    $sql = "UPDATE used_cars SET name=:name,price=:price,model=:model,type=:type,images=:images,specifications_en=:specifications_en,specifications_ar=:specifications_ar,km=:km WHERE id=:id";
                    $values = [
                        'name' => $name,
                        'price' => $price,
                        'model' => $year,
    ...
    {
        if(isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price']) && isset($_POST['year']) && !empty($_POST['year']) && isset($_POST['select']) && !empty($_POST['select']) && isset($_FILES['images']['name'][0]) && !empty($_FILES['images']['name'][0]) && isset($_POST['km']) && !empty($_POST['km']) && isset($_POST['specifications_en']) && !empty($_POST['specifications_en']) && isset($_POST['specifications_ar']) && !empty($_POST['specifications_ar']) && isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])){
            $images = [];
            $allowEx = ['png','jpg','jpeg'];
            $dir = 'assets/images/';
            $imgName = Validation::files("images",$allowEx,$dir,"used_car");
            ...
            elseif($imgName == 3){
                ...
                $sql = "INSERT INTO used_cars (name,price,model,type,images,km,specifications_en,specifications_ar,active_image) VALUES (:name,:price,:model,:type,:images,:km,:specifications_en,:specifications_ar,:image)";
                $values = [
                    'name' => $name,
                    'price' => $price,
                    'model' => $year,
