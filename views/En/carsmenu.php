...
<div>
<h1 class="text-center m-4">cars list</h1>
</div>
</section>
        ...
        <form class="m-auto">
            <ul class="list-unstyled d-flex p-0">
                <li class="position-relative">
                    ...
                    <span style="cursor:pointer" class="uk-position-center-right" uk-icon="icon: chevron-down; ratio: 1"></span>
                </li>
                <li><input id="carName" type="text" placeholder="search by car name"></li>
...
<script>
    ...
    function showComponents(){
        if("<?=$_GET['car']?>" == "all"){
            $("#showcars").html("<h3 class='row m-auto'>please select car brand to show</h3>");
        }
        else{
            $.getJSON("/CarsMenuComponents/rearrangeCars?"+path,function(data){
            if(data.cars == "no result"){
                $("#showcars").html("<h3 class='row m-auto'>There are no cars to display at the moment</h3>");
